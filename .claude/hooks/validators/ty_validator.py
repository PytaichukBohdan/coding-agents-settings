...
def log(message: str) -> None:
    log_dir = get_log_dir()
    log_file = log_dir / "ty_validator.log"
...
def main() -> None:
    try:
        ...
        log(f"Running ty check on: {file_path}")
        ...
        if result.returncode == 0:
            log(f"ty check passed: {file_path}")
        else:
            log(f"ty check issues in {file_path}:\n{result.stdout}\n{result.stderr}")
        ...
        log("ty not found in PATH - skipping check")
    except subprocess.TimeoutExpired:
        log("ty check timed out")
